...
class WatchdogConfig:
    poll_seconds: int = 15
    per_floor_timeout_seconds: int = 60
...
def _start_managed_process(name: str, node: dict[str, Any] | None, runtime: dict[str, Any]) -> bool:
    if not isinstance(node, dict) or not bool(node.get("enabled", False)):
        return False
    projects = runtime.get("projects") or {}
    default_root = Path(str(projects.get("sts2_cli_root") if name == "service" else projects.get("st2rl_root") or ROOT))
...
def _start_stack() -> None:
    subprocess.run(
        ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(ROOT / "scripts" / "start_stack.ps1")],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
        timeout=90,
    ...
    for history_path in _slot_history_paths(run_dir):
        for row in _read_history_rows(history_path)[-200:]:
            ...
            if termination_reason:
                ...
                if "deadlock" in lowered or "stuck" in lowered:
                    incidents.append(
                        {
                        **common,
                        "type": "deadlock_session",
                        "reason": f"session ended with termination_reason={termination_reason}",
                        "restart_required": False,
                    }
                )
    return incidents
...
def _restart_runtime(runtime: dict[str, Any], config: WatchdogConfig, reason: str, state: dict[str, Any]) -> None:
    ...
    print(f"[watchdog] restarting runtime: {reason}", flush=True)
    try:
        _post_dashboard("/api/control/stop", {})
    except Exception:
        pass
    _stop_service(runtime)
    _start_stack()
    _wait_for_url(f"{_dashboard_base_url()}/api/state", timeout_seconds=40)
    _wait_for_url(f"{_service_base_url(runtime)}/health", timeout_seconds=40)
    try:
        result = _post_dashboard("/api/control/start", {"total_timesteps": config.total_timesteps})
        print(f"[watchdog] training start result: {result}", flush=True)
    except Exception as exc:
        print(f"[watchdog] failed to restart training client: {exc}", flush=True)
    state["last_restart_at"] = _now_iso()
    state["restart_count"] = _safe_int(state.get("restart_count"), 0) + 1


def _ensure_training_running(snapshot: dict[str, Any], config: WatchdogConfig, state: dict[str, Any]) -> None:
    training = snapshot.get("training") or {}
...
def _ensure_runtime_healthy(runtime: dict[str, Any], config: WatchdogConfig, state: dict[str, Any]) -> None:
    try:
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            _ensure_runtime_healthy(runtime, config, state)
            _ensure_training_running(snapshot, config, state)
